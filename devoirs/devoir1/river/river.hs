...
data Passenger = Cabbage | Goat | Wolf deriving (Eq)
...
instance Show State where
  show (State lxs boat rxs) =
    do
      printf "%s\n%13s" separation "Rive gauche: " ++ displayList lxs ++ "\n"
      ++ printf "%13s" "Bateau: "
      ++ show boat
      ++ "\n"
...
test2 = State [] (Boat RightSide [Goat]) [Wolf, Cabbage]
...
getInput state = do
  s <- getLine
  if s == ":q" then putStrLn "Au revoir"
  else do
    let current = checkInput s state
    snd current
    getInput $ fst current

-- Le otherwise ne sera jamais utilisé faut il le mettre ?
